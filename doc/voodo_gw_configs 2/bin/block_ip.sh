...
if [ "$#" -eq 1 ]; then
    # Allow SSH traffic, log LAN destined traffic, then drop it
    iptables -A INPUT -s $1 -p tcp --dport 22 -i $LAN_NIC -j ACCEPT
    iptables -A INPUT -s $1 -i $LAN_NIC -j LOG --log-level 4 --log-prefix "Voodo In "
    iptables -A INPUT -s $1 -i $LAN_NIC -j DROP
fi
...
if [ "$#" -eq 1 ]; then
    # Drop all traffic from the source specified via command line arg
    iptables -A FORWARD -s $1 -i $LAN_NIC -j LOG --log-level 4 --log-prefix "Voodo Forward "
    iptables -A FORWARD -s $1 -i $LAN_NIC -j DROP
fi
...
# NAT rules
if [ "$#" -eq 1 ]; then
    # This logging rule confirms that the router is not actually allowing any traffic out
    # So, if you see this in the logs then bad things are happening and you should sort it
    iptables -t nat -A POSTROUTING -s $1 -o $WAN_NIC -j LOG --log-level 4 --log-prefix "Voodo Postrouting "
fi
