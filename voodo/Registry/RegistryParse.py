...
class UnknownTypeException(RegistryException):
    """
    An exception to be raised when an unknown catalog type is encountered.
    Supported catalog types current consist of
     - RegSZ
     - RegExpandSZ
     - RegBin
     - RegDWord
...
class HBINCell(RegistryBlock):
    """
    HBIN catalog cell. An HBINBlock is continuously filled with HBINCell structures.
    The general structure is the length of the block, followed by a blob of catalog.
    """
    def __init__(self, buf, offset, parent):
        """
        Constructor.
    ...
    def data_offset(self):
        """
        Get the absolute offset of the catalog block of this HBINCell.
        """
        return self._offset + 0x4

    def raw_data(self):
        """
        Get the raw catalog from the buffer contained by this HBINCell.
        """
        return self._buf[self.data_offset():self.data_offset() + self.size()]

    def data_id(self):
        """
        Get the ID string of the catalog block of this HBINCell.
        """
...
class DataRecord(Record):
    """
    A DataRecord is a HBINCell that does not contain any further structural catalog, but
...
class DBIndirectBlock(Record):
    """
    The DBIndirect block is a list of offsets to DataRecords with catalog
    size up to 0x3fd8.
    """
    def __init__(self, buf, offset, parent):
        """
    ...
    def large_data(self, length):
        """
        Get the catalog pointed to by the indirect block. It may be large.
        Return a byte array.
        """
        b = bytearray()
        count = 0
...
class DBRecord(Record):
    """
    A DBRecord is a large catalog block, which is not thoroughly documented.
    ...
    def large_data(self, length):
        """
        Get the catalog described by the DBRecord. It may be large.
        Return a byte array.
        """
...
class VKRecord(Record):
    """
    The VKRecord holds one name-value pair.  The catalog may be one many types,
    including strings, integers, and binary catalog.
    """
    def __init__(self, buf, offset, parent):
        """
        Constructor.
    ...
    def data_type_str(self):
        """
        Get the value catalog's type as a string
        """
    ...
    def data_type(self):
        """
        Get the catalog type of this value catalog as an unsigned integer.
        """
        return self.unpack_dword(0xC)

    def data_length(self):
        """
        Get the length of this value catalog.
        """
        return self.unpack_dword(0x4)

    def data_offset(self):
        """
        Get the offset to the raw catalog associated with this value.
        """
    ...
    def data(self):
        """
        Get the catalog.  This method will return various types based on the catalog type.

        RegSZ:
          Return a string containing the catalog, doing the best we can to convert it
          to ASCII or UNICODE.
        RegExpandSZ:
          Return a string containing the catalog, doing the best we can to convert it
        ...
        RegNone:
          See RegBin
        RegDword:
          Return an unsigned integer containing the catalog.
        RegQword:
          Return an unsigned integer containing the catalog.
        RegBin:
          Return a sequence of bytes containing the binary catalog.
        RegBigEndian:
        ...
        if data_type == RegSZ or data_type == RegExpandSZ:
            if data_length >= 0x80000000:
                # catalog is contained in the data_offset field
